...
function initializeGameBot() {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            createVisualUI();
            processInitQueue();
            logActivity("🚀 Enhanced Game Bot Framework initialized (DOM ready)");
        });
    } else {
        // DOM is already ready
        createVisualUI();
        processInitQueue();
        logActivity("🚀 Enhanced Game Bot Framework initialized");
    }
}
...
// Modified to use Alt+X or Alt+Ч instead of J/О
(function() {
    ...
    function autoChain_stopSpamming() {
        if (autoChain_interval) {
            ...
            logActivity("⏹️ Auto Chain spam stopped");
        }
    }
    ...
    setTimeout(() => {
        if (window.gameBotAddonManager) {
            ...
            logActivity("🔗 Auto Chain addon registered successfully");
        } else {
...
// This addon automatically cycles through keys 1-4 when toggled on by pressing Alt + X/Ж
(function() {
    ...
    // Handle keydown events - support Alt + X and Alt + Ж
    function onKeyDown(event) {
        // Check for Alt + X (English) or Alt + Ж (Russian) - same physical key
        if (event.altKey && (event.key.toLowerCase() === 'b' || event.key === 'и' || event.key === 'И')) {
...
(function() {
    ...
    function createRotationConfigUI() {
        ...
        rotationConfigButton.textContent = '⚙️';
    ...
    function toggleRotationMode() {
        ...
        if (rotationActive) {
            performRotation();
            const pattern = MOTION_PATTERNS[currentPattern];
            logActivity(`✅ ${pattern.name} включен`);
        } else {
            stopRotation();
            logActivity("❌ Выключен");
        }
    }
    ...
    function initialize() {
        ...
        logActivity("🎮 Аддон вращения готов!");
        const pattern = MOTION_PATTERNS[currentPattern];
        logActivity(`Текущий: ${pattern.name}`);
        logActivity("Alt+B/Alt+И - вкл/выкл");
    }
    ...
    function onKeyDown(event) {
        ...
        // Toggle rotation mode with Alt+B or Alt+И
        if (event.altKey && (key === 'v' || key === 'м')) {
    ...
    window.gameBotAddonManager.registerFeature('rotation-status', {
        label: 'Спин(Alt+М)', // Updated label to show new key combination
...
// This addon automatically holds the K key for exactly 3.875 seconds when toggled with Alt+C/Alt+С key combination
(function() {
    ...
    function initialize() {
        ...
        logActivity("Таймер удержания K инициализирован (Переключение Alt+C/Alt+С)");
    }
    ...
    // Handle keydown events - support Alt+C and Alt+С (Cyrillic C)
    function onKeyDown(event) {
        // Check for Alt+C (Latin) or Alt+С (Cyrillic)
        if (event.altKey && (event.key.toLowerCase() === 'c' || event.key === 'с' || event.key === 'С')) {
